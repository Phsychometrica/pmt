...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentUser = null; // Stores logged-in user info (role and UID)
    ...
    function login() {
        ...
        authenticate(email, password)
            .then(authResult => {
                currentUser = authResult; // Store the result (role and UID)
                showWelcomeScreen();
            })
            .catch(error => {
    ...
    function addNewUser() {
        ...
        if (currentUser?.role !== 'admin') {
            console.error('Non-admin attempted to add user');
            showAlert('error', 'Only admins can add users.');
            return;
        }
        ...
        const brandingName = document.getElementById('new-branding-name')?.value.trim();
        const brandingAddress = document.getElementById('new-branding-address')?.value.trim();
        const brandingPhone = document.getElementById('new-branding-phone')?.value.trim();

        if (!username || !password || !role || !brandingName || !brandingAddress || !brandingPhone) {
            console.error('Missing fields for new user');
        ...
        if (role !== 'admin' && !emailRegex.test(username)) {
            showAlert('error', 'Please enter a valid email address for the username.');
            return;
        }


        const success = window.createUser(username, password, role, brandingName, brandingAddress, brandingPhone);
        if (success) {
            showAlert('success', 'User added successfully.');
            document.getElementById('new-username').value = '';
            document.getElementById('new-password').value = '';
            document.getElementById('new-role').value = '';
            document.getElementById('new-branding-name').value = '';
            document.getElementById('new-branding-address').value = '';
            document.getElementById('new-branding-phone').value = '';
        } else {
    ...
    function updatePassword() {
        ...
        if (currentUser?.role !== 'admin') {
            console.error('Non-admin attempted to update password');
            showAlert('error', 'Only admins can update passwords.');
            return;
        }
        ...
        if (!username || !newPassword) {
            console.error('Missing fields for password update');
            showAlert('error', 'Please fill in all fields.');
            return;
        }
        ...
        const success = window.updateUserPassword(username, newPassword);
        if (success) {
            ...
            showAlert('error', 'Failed to update password. User not found.');
        }
    }
    ...
    function deleteUser() {
        ...
        if (currentUser?.role !== 'admin') {
            console.error('Non-admin attempted to delete user');
            showAlert('error', 'Only admins can delete users.');
            return;
        }
